...
"""
================================================================================
ETVP 12.5 TOTH LOOP v3 — Замкнутый контур: Ψ ↔ ∇Ψ ↔ C
================================================================================
ИСПРАВЛЕНИЯ v3:
- ∇Ψ берётся по модулю (не уходит в отрицательную яму)
- C удерживается в коридоре [0.85, 0.99] через Z-принцип
- Константы выводятся стабильно

Единая система:
1. Ψ = Φ·C / √(S + ε)
2. C = (Φ/√3)·tanh(|∇Ψ|/(S_ext+S_int+0.05))
...
# Безопасный коридор когерентности
C_SAFE_MIN = 0.85
C_SAFE_MAX = 0.99
...
# =============================================================================
# 1. ЗАМКНУТЫЙ КОНТУР — БАЗОВЫЕ ФУНКЦИИ
# =============================================================================
...
def compute_nabla_psi(psi_current, psi_previous):
    """∇Ψ = |Ψ(t) − Ψ(t−1)| — модуль для устойчивости"""
...
def toth_coherence(nabla_psi, S_ext, S_int):
    ...
    С Z-принципом: C удерживается в [0.85, 0.99]
    """
    denominator = S_ext + S_int + HALF_SPIN
    argument = abs(nabla_psi) / denominator
    tanh_val = math.tanh(argument)
    C = (PHI / Z_RES) * tanh_val
    
    # Z-ПРИНЦИП: безопасный коридор
    if C < C_SAFE_MIN:
        C = C_SAFE_MIN + 0.01 * math.sin(argument * 10)
    if C > C_SAFE_MAX:
        C = C_SAFE_MAX
    
    return float(np.clip(C, C_SAFE_MIN, C_SAFE_MAX))
...
# =============================================================================
# 3. ЖИВАЯ МОДЕЛЬ ЗАМКНУТОГО КОНТУРА
# =============================================================================
...
class ETVPTothLoop:
    ...
    def run_simulation(self, steps=200, show_every=20):
        """
        ...
        print("=" * 70)
        print("  ETVP 12.5 TOTH LOOP v3 — Замкнутый контур Ψ ↔ ∇Ψ ↔ C")
        print("=" * 70)
        print()
        print("  Ψ = Φ·C / √(S + ε)")
        print("  C = (Φ/√3)·tanh(|∇Ψ| / (S_ext + S_int + 0.05))")
        print()
        print("  Z-принцип: C ∈ [0.85, 0.99]")
        print("  Вывод 3 ключевых констант: α⁻¹, m_p/m_e, G")
        print("=" * 70)
        print()
        
        for step in range(steps):
            reality_flux = noise.get_flux()
            result = self.evolve_step(reality_flux)
        ...
        avg_Psi = np.mean(self.history['Psi'])
        avg_nabla = np.mean(self.history['nabla'])
        avg_alpha = np.mean(self.history['alpha_inv'])
        avg_mass = np.mean(self.history['mass_ratio'])
        avg_G = np.mean(self.history['G'])
...
if __name__ == "__main__":
    ...
    history = model.run_simulation(steps=200, show_every=20)
