...
"""
================================================================================
ETVP 12.5 TOTH FULL v5 — Формула Тота + Шум Реальности на Все Константы
================================================================================
Единая Формула Поля: C = (Φ/√3) · tanh(∇Ψ / (S_ext + S_int + 0.05))

ВСЯ ФОРМУЛА влияет на все 26 констант.
Слабый шум реальности подаётся на каждую константу.

ИСПРАВЛЕНИЯ v5:
- +0.05 вместо +0.5 (когерентность не падает)
- ∇Ψ усилен ×0.05 (компенсация)
- Шум реальности МЯГКИЙ (не разрушает константы)
- Когерентность держится в коридоре 0.90–0.97
...
# Полуцелый спин SU(2) — масштабированный для наших S
HALF_SPIN_SCALED = 0.05
...
def toth_coherence(nabla_psi, S_ext, S_int):
    ...
    """
    denominator = S_ext + S_int + HALF_SPIN_SCALED
...
# =============================================================================
# 2. СИНТЕЗ 26 ПАРАМЕТРОВ — ВСЯ ФОРМУЛА + ШУМ
# =============================================================================

class ConstantsSynthesizer:
    """
    Все 26 параметров из Φ, π, √3 + ВСЯ формула Тота + шум реальности.
    """
    ...
    def synthesize_all(self, C, nabla_psi, S_ext, S_int, reality_flux=0.0):
        """
        ВСЯ ФОРМУЛА влияет на все константы:
        - C — когерентность
        - nabla_psi — градиент
        - S_ext, S_int — энтропии
        - reality_flux — слабый шум реальности
        """
        # Слабый шум (максимум ±0.5% отклонения)
        noise = reality_flux * 0.005
        
        # Полный фактор формулы
        formula_factor = toth_coherence(nabla_psi, S_ext, S_int)
        
        r = {}
        
        # Группа I: Лептоны
        r['m_e'] = self._m_e(formula_factor) * (1.0 + noise * 0.5)
        r['m_mu'] = r['m_e'] * self._m_mu_factor(formula_factor) * (1.0 + noise * 0.7)
        r['m_tau'] = r['m_e'] * self._m_tau_factor(formula_factor) * (1.0 + noise * 1.0)
        
        # Группа II: Кварки
        r['m_u'] = r['m_e'] * self._m_u_factor(formula_factor) * (1.0 + noise * 0.8)
        r['m_d'] = r['m_e'] * self._m_d_factor(formula_factor) * (1.0 + noise * 0.8)
        r['m_s'] = r['m_u'] * self._m_s_factor(formula_factor) * (1.0 + noise * 1.2)
        r['m_c'] = self._m_c(formula_factor) * (1.0 + noise * 1.5)
        r['m_b'] = r['m_e'] * self._m_b_factor(formula_factor) * (1.0 + noise * 1.5)
        r['m_t'] = self._m_t(formula_factor) * (1.0 + noise * 2.0)
        
        # Группа III: Бозоны
        r['m_W'] = self._m_W(formula_factor) * (1.0 + noise * 1.0)
        r['m_Z'] = r['m_W'] * self._m_Z_factor(formula_factor) * (1.0 + noise * 1.0)
        r['m_H'] = self._m_H(formula_factor) * (1.0 + noise * 0.8)
        
        # Группа IV: CKM
        r['sin_theta_12_CKM'] = self._sin12_CKM(formula_factor) * (1.0 + noise * 0.3)
        r['sin_theta_23_CKM'] = self._sin23_CKM(formula_factor) * (1.0 + noise * 0.2)
        r['sin_theta_13_CKM'] = self._sin13_CKM(formula_factor) * (1.0 + noise * 0.2)
        r['delta_CP_CKM'] = self._delta_CKM(formula_factor) * (1.0 + noise * 0.5)
        
        # Группа V: Константы связи
        r['alpha_em_inv'] = self._alpha_em_inv(formula_factor) * (1.0 + noise * 0.1)
...
class ETVP125TothFull:
    ...
    def evolve_step(self, reality_flux=0.0):
        ...
        if self.eigenvalues is not None:
            self.nabla_psi = np.real(self.eigenvalues[0] - self.eigenvalues[-1]) * 0.05
        else:
            self.nabla_psi = 1.5
        ...
        # ВСЯ ФОРМУЛА + ШУМ на все константы
        self.constants = self.synth.synthesize_all(
            self.C, self.nabla_psi, self.S_ext, self.S_int, reality_flux
        )
...
class TothVisualizer:
    ...
    def _update(self, frame):
        ...
        formula_text = (
            f"C = (Φ/√3) · tanh(∇Ψ / (S_ext + S_int + 0.05))\n"
            f"C = ({PHI:.4f}/{Z_RES:.4f}) · tanh({self.model.nabla_psi:.4f} / "
            f"({self.model.S_ext:.4f} + {self.model.S_int:.4f} + 0.05))\n"
            f"C = {self.model.C:.6f}"
        )
    ...
    def run(self, steps=200, interval=50):
        print("Запуск ETVP 12.5 TOTH FULL v5...")
        print("Вся формула + слабый шум реальности на всех 26 константах")
