...
ИСПРАВЛЕНИЯ:
- Замедленная анимация (delay=0.3)
- Надёжная очистка экрана
- Пошаговый режим (Enter для следующего кадра)
- Чёткий вывод всех блоков
...
class ASCIIVisualizer:
    """Полная ASCII-визуализация с пошаговым режимом."""
    
    def __init__(self, model, width=90):
        self.model = model
        self.width = width
        self.history_C = deque(maxlen=40)
        self.history_S = deque(maxlen=40)
    ...
    def _render_spectrum(self):
        ...
        lines = []
        lines.append("┌─ СПЕКТР E₈ ──────────────────────────────────────────────┐")
        lines.append("│ λ    Re                │ Im                │")
        lines.append("├──────────────────────────────────────────────────────────┤")
        
        for i in range(len(eig)):
            bar_re = self._bar(re_vals[i], max_re, 20)
            bar_im = self._bar(im_vals[i], max_im, 20)
            lines.append(f"│ {i+1:02d}  [{bar_re}] [{bar_im}] │")
    ...
    def _render_constants(self):
        ...
        lines = []
        lines.append("┌─ 26 ПАРАМЕТРОВ (отклонение от CODATA) ──────────────────┐")
    ...
    def render(self, step=None, total=None):
        """Полный рендер одного кадра."""
        self._clear()
        
        print("═" * self.width)
        print("  ETVP 12.5 INFINITUM ASCII — ЖИВАЯ ДИНАМИКА ПОЛЯ")
        print("═" * self.width)
        ...
        print()
        print("═" * self.width)
        print("  Enter — следующий шаг | 'q' — выход | 'auto' — автозапуск")
        print("═" * self.width)
    
    def run(self, steps=200, mode='auto', delay=0.3):
        """
        Запуск визуализации.
        
        Args:
            steps: количество шагов
            mode: 'auto' — автоматический, 'step' — пошаговый (Enter)
            delay: задержка между кадрами в режиме 'auto'
        """
        print("Запуск ETVP 12.5 ASCII-визуализации...")
        time.sleep(1)
        ...
        print("═" * self.width)
        print(f"  Шагов выполнено: {self.model.step_counter}")
        print(f"  Средняя C: {np.mean(list(self.history_C)):.4f}")
        print(f"  α⁻¹ = {self.model.constants['alpha_em_inv']:.6f}")
        print(f"        CODATA: {CODATA['alpha_em_inv']}")
        print(f"  m_W = {self.model.constants['m_W']:.1f} МэВ")
        print(f"        CODATA: {CODATA['m_W']} МэВ")
        print(f"  m_Z = {self.model.constants['m_Z']:.1f} МэВ")
        print(f"        CODATA: {CODATA['m_Z']} МэВ")
        print("═" * self.width)
...
if __name__ == "__main__":
    ...
    # Выбор режима:
    # mode='auto' — автоматический показ (delay=0.3 сек)
    # mode='step' — пошаговый (нажмите Enter для следующего кадра)
